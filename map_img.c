...
/*   Updated: 2024/11/19 16:36:07 by brendon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    data->wall_img = mlx_xpm_file_to_image(data->mlx, "img/wall.xpm", &data->img_width, &data->img_height);
    if (!data->wall_img)
    {
        printf("Error loading wall image\n");
        exit(1);
    }
    data->floor_img = mlx_xpm_file_to_image(data->mlx, "img/ground.xpm", &data->img_width, &data->img_height);
    if (!data->floor_img)
    {
        printf("Error loading floor image\n");
        exit(1);
    }
    data->player_img = mlx_xpm_file_to_image(data->mlx, "img/player.xpm", &data->img_width, &data->img_height);
    if (!data->player_img)
    {
        printf("Error loading player image\n");
        exit(1);
    }
    data->exit_img = mlx_xpm_file_to_image(data->mlx, "img/exit.xpm", &data->img_width, &data->img_height);
    if (!data->exit_img)
    {
        printf("Error loading exit image\n");
        exit(1);
    }
    data->colectible_img = mlx_xpm_file_to_image(data->mlx, "img/c1.xpm", &data->img_width, &data->img_height);
...
void	open_window(t_data *data)
{
	...
	data->win = mlx_new_window(data->mlx, data->win_width, data->win_height, "Move Image");
	load_img(data);
	put_image_to_window(data);
